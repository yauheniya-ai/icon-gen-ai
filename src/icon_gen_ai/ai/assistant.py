...
class IconAssistant:
    ...
    def _auto_detect_provider(self) -> Optional[BaseLLMProvider]:
        ...
        if anthropic_key:
            try:
                ...
                return AnthropicProvider(api_key=anthropic_key, base_url=anthropic_base)
            except ImportError:
        ...
        if openai_key:
            try:
                ...
                return OpenAIProvider(api_key=openai_key, base_url=openai_base)
            except ImportError:
        ...
        if huggingface_key:
            try:
                ...
                return HuggingFaceProvider(
                    api_key=huggingface_key, base_url=huggingface_base
                )
            except ImportError:
    ...
    def _get_from_cache(self, cache_key: str) -> Optional[LLMResponse]:
        ...
        if cache_file.exists():
            try:
                ...
                suggestions = [IconSuggestion(**s) for s in data.get("suggestions", [])]
    ...
    def discover_icons(
        ...
        # Query LLM
        print(
            f"Querying Iconify using {self.provider.get_provider_name()} (model: {self.provider.model})..."
        )
